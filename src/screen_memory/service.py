...
from screen_memory.screen_db import ScreenMemoryDB
...
def _screen_memory_dir() -> Path:
    return Path(SCHEDULE_STATE_PATH).expanduser().parent


def _state_path() -> Path:
    return Path(SCHEDULE_STATE_PATH).expanduser()


def _lock_path() -> Path:
    return Path(PIPELINE_LOCK_FILE).expanduser()
...
def _inject_runtime_config(
    ...
    model = _configured_model(gui_config)
    if not model:
        return None
    model_config = gui_config.get("model", {})
    runtime = model_config if isinstance(model_config, dict) else {}
    base_url = runtime.get("base_url") or ""
    api_key = (
        resolve_api_key(runtime, default_env="LLM_API_KEY")
        or os.environ.get("OPENAI_API_KEY")
        or os.environ.get("DEEPSEEK_API_KEY")
        or "dummy"
    )
    api_key_env = str(runtime.get("api_key_env") or "LLM_API_KEY").strip()
    for section_name in (
        "segment_generation",
        ...
        section["llm_base_url"] = base_url
        section["api_key"] = api_key
        section["api_key_env"] = api_key_env
        section["llm_api_key_env"] = api_key_env
    try:
        from openai import OpenAI

        return OpenAI(base_url=base_url or None, api_key=api_key)
    except Exception as exc:
        logger.warning("Screen memory could not create LLM client: %s", exc)
        return None
...
def _run_fact_extraction(
    ...
    if custom_start_time is not None:
        previous = _as_utc(custom_start_time)
    else:
        previous = _parse_state_time(
            state.get("last_fact_extraction_at") or state.get("last_ingest_at")
        )
        if previous is None:
            previous = now - timedelta(
                minutes=max(1, int(schedule.get("initial_lookback_minutes", 30)))
            )
    end = _as_utc(custom_end_time) if custom_end_time is not None else now
    stats = manager.update_screen_facts_table(
        start_time_str=_format_utc_time(previous),
        end_time_str=_format_utc_time(end),
        incremental=True,
    )
    if stats is not None and custom_start_time is None:
...
def _normalize_force_phase(force_phase: Optional[str]) -> Optional[str]:
    if not force_phase or force_phase == "all":
        return None
    aliases = {
        "ingest": "fact_extraction",
        "clean": "fact_extraction",
        "cluster": "observation",
        "fact_clustering": "observation",
        "observations": "observation",
        "tasks": "task",
    }
    return aliases.get(force_phase, force_phase)
...
def run_screen_memory_due_work(
    *,
    ...
    manager_config = load_screen_memory_config(gui_config)
    force_phase = _normalize_force_phase(force_phase)
    if not manager_config.get("enabled") and custom_start_time is None:
    ...
    try:
        ...
        fact_extraction_due = (
            force_phase == "fact_extraction"
            or custom_start_time is not None
            or (
                force_phase is None
                and _is_due(
                    ...
                    timedelta(
                        minutes=max(
                            1,
                            int(schedule["fact_extraction_interval_minutes"]),
                        )
                    ),
                )
            )
        ...
        llm_client = _inject_runtime_config(manager_config, gui_config)
        ...
        if fact_extraction_due:
            phases["fact_extraction"] = _run_fact_extraction(
                manager,
                state,
                current,
