...
from screen_memory.utils import *
from screen_memory.config import load_screen_memory_config
from screen_memory.screen_db import ScreenMemoryDB
from screen_memory.prompts import *
...
class ScreenMemoryManager:
    ...
    def call_json_llm(self, system_prompt, user_prompt, config):
        ...
        api_key_env = config.get("llm_api_key_env") or config.get("api_key_env") or "LLM_API_KEY"
        api_key = str(config.get("api_key") or "").strip() or os.environ.get(api_key_env, "").strip()
    ...
    def _load_raw_data(self, start_time, end_time, min_quality):
        ...
        if not kept_records:
            stats.update(self.screen_db.get_workstream_stats())
            return stats
