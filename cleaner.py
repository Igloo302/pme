...
def load_screen_memory_config():
    import yaml
    from smgui.config import SCREENPIPE_DB, OPENCHRONICLE_DB, OUTPUT_DB, CONFIG_PATH
    ...
    sm_settings = yaml_data.get('screen_memory', {})
    db_settings = yaml_data.get('database', {})
    
    config = {
        "enabled": sm_settings.get("enabled", True),
        "schedule": {
            "ingest_interval_minutes": sm_settings.get("ingest_interval_minutes", 30),
            "fact_clustering_interval_hours": sm_settings.get("fact_clustering_interval_hours", 2),
            "observation_interval_hours": sm_settings.get("observation_interval_hours", 24),
            "initial_lookback_minutes": sm_settings.get("initial_lookback_minutes", 30),
        },
        "database": {
            "screenpipe_db": os.path.expanduser(db_settings.get("screenpipe_db", SCREENPIPE_DB)),
            "openchronicle_db": os.path.expanduser(db_settings.get("openchronicle_db", OPENCHRONICLE_DB)),
            "cleaned_db": os.path.expanduser(db_settings.get("cleaned_db", OUTPUT_DB)),
        },
        "openchronicle": {
    ...
    llm_base_url = model_settings.get('base_url')
    llm_api_key = model_settings.get('api_key')
    llm_provider = model_settings.get('provider')

    # Merge model settings into each generation config
    for gen_key in [
        "segment_generation",
        "window_workstream_generation",
        "task_workstream_generation",
        ...
        if gen_key in config:
            config[gen_key]["llm_model"] = llm_model
            config[gen_key]["llm_base_url"] = llm_base_url
            config[gen_key]["api_key"] = llm_api_key
            config[gen_key]["llm_provider"] = llm_provider
    ...
    def call_json_llm(self, system_prompt, user_prompt, config):
        ...
        if self.llm_client is not None:
            ...
            try:
                ...
                raise RuntimeError(f"Direct LLM API call failed: {str(e)}") from e
        if not content:
            reasoning = None
            if choices:
                if self.llm_client is not None:
            ...
            if reasoning:
                raise RuntimeError(f"SMGUI LLM call returned no content, but had reasoning process: {reasoning[:500]}...")
            raise RuntimeError("SMGUI LLM call returned no content")
        try:
